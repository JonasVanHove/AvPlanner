...
export default function TeamSharePage({ params }: TeamSharePageProps) {
  ...
  const handleDateNavigation = useCallback((newDate: Date) => {
    ...
    const newUrl = `/team/${resolvedParams.slug}/share/${year}/${month}/${day}${weeksQuery}`
    const currentPath = window.location.pathname + window.location.search
    if (currentPath !== newUrl) {
      window.history.pushState(null, "", newUrl)
      setUrlDate(newDate)
    }
  }, [resolvedParams.slug, weeksQuery])
    ...
    try {
      ...
      try {
        ...
        if (invite && resolvedParams.slug !== invite) {
          ...
          router.replace(`/team/${invite}${suffix || ""}${weeksQuery}`)
          return
        }
      } catch {}
