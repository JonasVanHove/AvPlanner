...
interface LocaleSharePageParams {
  locale: string
  slug: string
}
...
export default function LocaleTeamSharePage({ params }: { params: Promise<LocaleSharePageParams> }) {
  ...
  const handleDateNavigation = useCallback((newDate: Date) => {
    ...
    const newUrl = `/${locale}/team/${resolvedParams.slug}/share/${year}/${month}/${day}${weeksQuery}`
    const currentPath = window.location.pathname + window.location.search
    if (currentPath !== newUrl) {
      window.history.pushState(null, "", newUrl)
      setUrlDate(newDate)
    }
  }, [locale, resolvedParams.slug, weeksQuery])
    ...
    try {
      ...
      try {
        ...
        if (invite && resolvedParams.slug !== invite) {
          ...
          const prefix = `/${locale}/team/${resolvedParams.slug}`
